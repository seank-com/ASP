...
<%
	'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	' Displays random wit (requires wit.txt)
	'
	Option Explicit
	...
	' grab that
	if qsI = "" then
		if Application("Wit-Index") = "" then
			qsI = 1
		else
			qsI = Application("Wit-Index")
		end if
	end if
	...
	set objStream = objFile.OpenTextFile(Server.MapPath("/data/wit.txt"))
